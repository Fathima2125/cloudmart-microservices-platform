# CloudMart AWS ALB Ingress

## Purpose

This phase exposes the CloudMart application running on Amazon EKS to the internet using AWS Application Load Balancer.

Instead of using port-forwarding, traffic now flows through:

Internet → AWS ALB → Kubernetes Ingress → CloudMart Services → RDS/ElastiCache
...
- Verified CloudMart website through the ALB DNS name.
...
kubectl describe ingress cloudmart-alb-ingress -n cloudmart
Result

CloudMart successfully runs as a cloud-native application on AWS EKS and is publicly accessible through AWS Application Load Balancer.


### Screenshots to take:

1. kubectl get ingress -n cloudmart
![alt text](<screenshots/kubectl get ingress -n cloudmart.png>)
...
4. kubectl describe ingress cloudmart-alb-ingress -n cloudmart

![alt text](<screenshots/kubectl describe ingress cloudmart-alb-ingress -n cloudmart.png>)
...
7. Browser showing CloudMart homepage through ALB DNS

![alt text](<screenshots/ Browser showing CloudMart homepage through ALB DNS.png>)
