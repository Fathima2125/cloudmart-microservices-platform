# Redeploy CloudMart After Terraform Destroy

Use this guide after you previously ran `terraform destroy` and want to bring CloudMart back online.
...
```bash
kubectl apply -f infra/argocd/cloudmart-application.yaml
```
...
```bash
kubectl patch application cloudmart \
  -n argocd \
  --type merge \
...
## Step 13: Check CloudMart Deployment
...
kubectl wait --for=condition=complete job/cloudmart-rds-init -n cloudmart --timeout=180s
...
```bash
ALB_DNS=$(kubectl get ingress cloudmart-alb-ingress \
  -n cloudmart \
