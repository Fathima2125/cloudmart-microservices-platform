...
CloudMart is a cloud-native electronics e-commerce platform built using a microservices architecture. The application has been containerized using Docker and orchestrated using Docker Compose.
...
```bash
docker exec -it cloudmart-postgres psql -U postgres -d cloudmart_db
```
