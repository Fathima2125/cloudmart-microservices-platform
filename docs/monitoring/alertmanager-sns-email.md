# CloudMart Alertmanager SNS Email Receiver

This phase connects CloudMart Prometheus alerts to AWS SNS and email.
...
```text
AlertmanagerConfig: cloudmart-alertmanager-config
Receiver: cloudmart-sns-alerts
```
...
IAM role
  |
  v
cloudmart-monitoring-alerts-publish-policy
```
...
```bash
helm template cloudmart infra/helm/cloudmart -n cloudmart | kubectl apply -f -
```
...
kubectl get alertmanagerconfig cloudmart-alertmanager-config -n cloudmart -o yaml
```
...
```text
sns_configs
topic_arn: arn:aws:sns:us-east-1:506098131053:cloudmart-monitoring-alerts
```
