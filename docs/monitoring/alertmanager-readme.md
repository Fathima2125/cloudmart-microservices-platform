# CloudMart Alertmanager Setup

This document explains the Alertmanager phase of CloudMart monitoring.

## Goal

Prometheus can detect problems, but Alertmanager is responsible for sending notifications.

CloudMart alerting flow:
...
Instead of using AWS access keys, CloudMart uses IRSA.
...
eksctl create iamserviceaccount \
  ...
  --attach-policy-arn arn:aws:iam::506098131053:policy/cloudmart-monitoring-alerts-publish-policy \
...
```bash
kubectl get prometheusrule -n cloudmart --show-labels
```
...
Use this page to see CloudMart alert rules.
...
curl -X POST http://127.0.0.1:19093/api/v2/alerts \
  ...
  --data '[{
    "labels": {
      "alertname": "CloudMartTestAlert",
      "namespace": "cloudmart",
      "severity": "warning",
      "service": "test"
    },
    "annotations": {
      "summary": "CloudMart test alert",
...
1. kubectl get prometheusrule -n cloudmart --show-labels
![alt text](<../aws/screenshots/kubectl get prometheusrule -n cloudmart --show-labels.png>)


2. kubectl get alertmanagerconfig -n cloudmart
![alt text](<../aws/screenshots/kubectl get alertmanagerconfig -n cloudmart.png>)
...
4. kubectl get application cloudmart -n argocd
![alt text](<../aws/screenshots/kubectl get application cloudmart -n argocd.png>)
...
7. AWS SNS topic: cloudmart-monitoring-alerts
![alt text](<../aws/screenshots/AWS SNS topic- cloudmart-monitoring-alerts.png>)
...
```text
Configured Alertmanager for CloudMart using Prometheus alert rules and AWS SNS email notifications. Alert rules detect service downtime, high 5xx error rate, high latency, pod restarts, pod readiness issues, and high memory usage. Alertmanager routes CloudMart alerts to an SNS topic, and SNS delivers notifications to email. IRSA is used so Alertmanager can publish to SNS without storing AWS credentials in Kubernetes.
```
